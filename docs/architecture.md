...
| **Graph** | `backend/app/graph/` | Двудольный граф, автопостроение, альтернативы |
...
7. Продолжает раскрывать ингредиенты рекурсивно.
8. При необходимости переключает **альтернативный рецепт** того же предмета.
9. Нажимает «Построить цепочку автоматически» — backend рекурсивно достраивает граф.
10. Задаёт целевой выход (100 пластин/мин) — backend считает машины и потребление ресурсов.
...
| POST | `/graph/auto-build` | Автопостроение цепочки от целевого предмета |
| POST | `/graph/calculate` | Расчёт производительности для графа / цели |
| GET | `/docs` | Swagger UI |
